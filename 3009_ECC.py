...
move_citizen = 0  #move_citizen = 1이면 시민 옮기기 수
citizen_min_area = 30000
...
h_max = [255, 65, 196, 138, 80]
h_min = [55, 0, 158, 53, 0]

s_max = [162, 200, 223, 210, 155]
s_min = [114, 140, 150, 125, 90]

v_max = [77, 151, 239, 114, 141]
v_min = [0, 95, 104, 74, 104]
        ...
        for i in color_num:  # 5가지 색(color_num)에 대하여 영상 인식
            ...
            if len(cnts) > 0:  # cnts에 저장된 값이 있으면(now_color와 동일한 색의 물체가 있으면) 아래코드 수행
                ...
                if Area[i] > min_area[now_color]:  # 물체의 넓이가 임계값보다 크면 아래 코드 수행
                    ...
                    # 라인이 인식되었고, line_on_off=1이고, char_detect_onoff = 0d이면 라인트레이싱 시작
                    # now_color = 3(녹색선)으로 테스트 함. 추후에 수정 필요
                    if now_color == 0 and line_onoff == 1 and char_detect_onoff == 0 and check_contamination == 0:
                        ...
                        # 라인이 끊겨 있을 때의 동작
                        if line_angle < -30 and X_255_point[i] < 110:  # 라인이 왼쪽으로 비스듬하게 위치
                            draw_str2(frame_roi, (3, 120), 'turn left 90 degree')
                            # print("\n", XY_Point2[0])
                            print("라인이 왼쪽으로 90도 꺾임")
                            line_onoff = 0  # 라인 트레이싱 중지
                            char_detect_onoff == 1  # 문자 인식
                            roi_num = 1  # 관심 영역: 화면 상단 반쪽으로 설정 -> 방 알파벳 인식 하려고..
                            break

                        elif line_angle > 30 and X_255_point[i] > 150:  # 라인이 오쪽으로 비스듬하게 위치
                            draw_str2(frame_roi, (3, 120), 'turn right 90 degree')
                            # print("\n", XY_Point2[0])
                            print("라인이 오쪽으로 90도 꺾임")
                            line_onoff = 0  # 라인 트레이싱 중지
                            char_detect_onoff == 1  # 문자 인식
                            roi_num = 1  # 관심 영역: 화면 상단 반쪽으로 설정 -> 방 알파벳 인식 하려고..
                            break
                        ...
                        # 라인 트레이싱 수행
                        if line_angle < -10 and line_angle > -30:  # 라인이 왼쪽으로 비스듬하게 위치
                            draw_str2(frame_roi, (3, 120), 'turn left 20 degree')
                            print("왼쪽턴 20")
                            TX_data(serial_port, 106)  # 명령_왼쪽턴 20도
                            for k in range(5):
                                TX_data(serial_port, 10)
                            k = 0
                            break
                        elif line_angle > 10 and line_angle < 30:  # 라인이 오른쪽으로 비스듬하게 위치
                            draw_str2(frame_roi, (3, 120), 'turn right 20 degree')
                            print("오른쪽턴 20")
                            TX_data(serial_port, 105)  # 명령_오른쪽턴 20도
                            for k in range(5):
                                TX_data(serial_port, 10)
                            k = 0
                            break

                        if X_255_point[i] > 0 and X_255_point[i] < 117:  # 라인이 왼쪽에 위치하면
                            ...
                            for k in range(5):
                                TX_data(serial_port, 10)
                            k = 0
                        elif X_255_point[i] >= 117 and X_255_point[i] <= 147:  # 라인이 중심에 위치하면
                            ...
                            for k in range(5):
                                TX_data(serial_port, 10)
                            k = 0
                        elif X_255_point[i] > 147 and X_255_point[i] <= 255:  # 라인이 오른쪽에 위치하면
                            ...
                            for k in range(5):
                                TX_data(serial_port, 10)
                            k = 0
                        elif X_255_point[i] > 147 and Y_255_point[i] > 100 and Y_255_point[i] < 157 and Area[
                            i] > 15000 and direction == 105:
                            print("오른쪽 문 열고 탈출")
                            ...
                            roi_num = 1
                            break
                        elif X_255_point[i] < 117 and Y_255_point[i] > 100 and Y_255_point[i] < 157 and Area[
                            i] > 15000 and direction == 0:
                            print("왼쪽 문 열고 탈출")
                    ...
                    # 시민 옮기기
                    if move_citizen == 1 and now_color == citizen_color 
                    and char_detect_onoff == 0:
                        if X_255_point[i] > 0 and X_255_point[i] < 117 and Area[i] < citizen_min_area:  # 시민이 왼쪽에 위치하면
                            draw_str2(frame_roi, (3, 120), 'go left')
                            print("왼쪽으로 이동")
                            TX_data(serial_port, 115)  # 명령_왼쪽으로 이동
                            for k in range(5):
                                TX_data(serial_port, 10)
                            k = 0
                        elif X_255_point[i] >= 117 and X_255_point[i] <= 147 and Area[i] < citizen_min_area:  # 시민이 중심에 위치하면
                            draw_str2(frame_roi, (3, 120), 'go straight')
                            print("직진")
                            TX_data(serial_port, 110)  # 명령_직진
                            for k in range(5):
                                TX_data(serial_port, 10)
                            k = 0
                        elif X_255_point[i] > 147 and X_255_point[i] <= 255 and Area[i] < citizen_min_area:  # 시민이 오른쪽에 위치하면
                            draw_str2(frame_roi, (3, 120), 'go right')
                            print("오른쪽으로 이동")
                            TX_data(serial_port, 120)  # 명령_오른쪽으로 이동
                            for k in range(5):
                                TX_data(serial_port, 10)
                            k = 0
                        elif X_255_point[i] >= 117 and X_255_point[i] <= 147 and Area[i] > citizen_min_area:  # 시민이 바로 앞에 위치하면
                            draw_str2(frame_roi, (3, 120), 'catch the citizen')
                            print("직진")
                            TX_data(serial_port, 20)  # 명령_시민들기
                            for k in range(5):
                                TX_data(serial_port, 10)
                            k = 0
                    ...
                    # 문자 인식
                    if (
                            now_color == 1 or now_color == 2 or now_color == 4) and char_detect_onoff == 1 and check_contamination == 0:
                        ...
                        # show the character
                        if check_cardinal_point_1 == 0:  # 시작위치의 방위 체크 하기 전이면
                            if strFinalString == 'N':
                                cardinal_point = 101
                        ...
                        if check_cardinal_point_1 == 1 and room_1 == 0:  # 화살표 방향 체크
                            if strFinalString == 'R':  # 화살표 방향 = 오른쪽
                                direction = 105
                                ...
                                print("serial count end")
                                break
                            elif strFinalString == 'L':  # 화살표 방향 = 왼쪽
                                direction = 106
                                TX_data(serial_port, direction)
                                char_detect_onoff = 0
                                line_onoff = 1
                                ...
                                print("serial count end")
                                break
                        elif check_cardinal_point_1 == 1 and check_dir == 1:  # 방 이름 인식
                            if strFinalString == 'A':
                                room_name = 125
                                TX_data(serial_port, room_name)  # 방 이름 전송
                                citizen_color = now_color  # 구출해야하는 시민 색
                                check_contamination = 1  # 다음 loop에서 방 확진지역여부 체크
                                roi = 3  # 확진지역 체크하기 위해 관심영역 재설정
                                char_detect_onoff = 0
                                print(
                                    "\ncharacter area: " + str(contourWithData.fltArea) + "\n" + strFinalString)
                                print("char_color: " + str(now_color))
                                ...
                                print("serial count end")
                                break
                            elif strFinalString == 'C':
                                room_name = 135
                                TX_data(serial_port, room_name)  # 방이름 전송
                                citizen_color = now_color  # 구출해야하는 시민 색
                                check_contamination = 1  # 다음 loop에서 방 확진지역여부 체크
                                roi = 3  # 확진지역 체크하기 위해 관심영역 재설정
                                char_detect_onoff = 0
                                ...
                                break
                            break
                        elif go_exit == 1 and now_color == 4 and check_cardinal_point_2 == 0:
                            if strFinalString == 'N':
                                cardinal_point = 101
                                TX_data(serial_port, cardinal_point)
                    ...
                    if now_color == 4:
                        if check_contamination == 1:  # 확진지역 체크
                            if Area.argmax() == 3:  # 방이 확진지역이 아니면
                                for serial_count in range(count_max):
                                    TX_data(serial_port, 160)  # 명령_안전지역 말하기
                                print("serial count end")
                                ...
                                for serial_count in range(count_max):
                                    TX_data(serial_port, 165)  # 명령_확진구역 말하기
                                print("serial count end")
                                check_contamination = 0
                                contamination = 1
                                roi_num = 2
